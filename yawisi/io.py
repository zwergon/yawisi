...
def from_bts(filename, tdecimals=8) -> WindField:
    ...
    with open(filename, mode="rb") as f:
        ...
        for it in range(nt):
            Buffer = (
                np.frombuffer(f.read(2 * 3 * ny * nz), dtype=np.int16)
                .astype(np.float32)
                .reshape([3, ny, nz], order="F")
            )
            u[:, it, :, :] = Buffer
            if nTwr > 0:
    ...
    params.sample_time = np.round(
        dt, tdecimals
    )  # dt is stored in single precision in the TurbSim output
    print(params)
